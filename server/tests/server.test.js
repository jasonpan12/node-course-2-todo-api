...
describe('POST /todos', () => {
  it('should create a new todo', (done) => {
    ...
    request(app)
    .post('/todos')
    .set('x-auth', users[0].tokens[0].token) // set x-auth header to first user, first token and token entry
    .send({text})
    .expect(200)
    .expect((res) => {
...
describe('DELETE /todos/:id', () => {
  it('should remove a todo', (done) => {
    var hexId = todos[1]._id.toHexString(); // remove the second todo, which belongs to the second user
  ...
  it('should not remove a todo created by other', (done) => {
    ...
    request(app)
      .delete(`/todos/${hexId}`)
      .set('x-auth', users[0].tokens[0].token) // delete first as second user
      .expect(404)
      .end((err, res) => {
        if(err) {
          return done(err);
        ...
        Todo.findById(hexId).then((todos) => {
          expect(todos).toBeTruthy(); // should still exist after attempt to delete
          done();
        }).catch((e) => done(e));
      })
  });
...
describe('PATCH /todos/:id', () => {
  it('should update the todo', (done) => {
    ...
    request(app)
      .patch(`/todos/${hexId}`)
      .set('x-auth', users[0].tokens[0].token) // update user1's todo as user1
      .send({
        completed: true,
        text
      })
  ...
  it('should not update the todo created by another', (done) => {
    ...
    request(app)
      .patch(`/todos/${hexId}`)
      .set('x-auth', users[1].tokens[0].token) // update user1's todo as user1
      .send({
        completed: true,
        text
      })
  ...
  it('should clear completedAt when todo is not completed', (done) => {
    ...
    var hexId = todos[1]._id.toHexString(); // 2nd todo, so use user 2's token
    var text = 'This should be the new text';

    request(app)
      .patch(`/todos/${hexId}`)
      .set('x-auth', users[1].tokens[0].token) // use user 2's todo
      .send({
        completedAt: null,
        completed: false,
        text
  ...
  it('should create a user', (done) => {
    ...
    request(app)
      ...
      .expect((res) => { // must use bracket because of dash
        expect(res.headers['x-auth']).toBeTruthy(); // expect x-auth header to be present
        expect(res.body._id).toBeTruthy(); // expect a user Id to be returned
      ...
      .end((err) => {
        ...
        User.findOne({email}).then((user) => {
          expect(user).toBeTruthy(); // expect user to be found in db
          expect(user.password).not.toBe(password); // expect hashed password toNotBe unhashed pw
          done();
  ...
  it('should login user and return auth token', (done) => {
    ...
    .expect((res) => { // expect an x-auth header to exist.
      ...
      User.findById(users[1]._id).then((user) => {
        expect(user.toObject().tokens[1]).toMatchObject({ // the found user should match the request's token
          access: 'auth',
